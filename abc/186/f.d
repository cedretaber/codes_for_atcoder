...
void main()
{
    ...
    auto min_y = H + 1;
    while (!ss.empty && ss.front.x == 1) {
        min_y = min(min_y, ss.front.y);
        ss.popFront();
    }

    auto segt = Segtree!(int, "a + b", () => 0)(H + 1);
    foreach (y; 1..min_y) segt.set(y, 1);
    long res = segt.allProd();
    bool xend;
    foreach (x; 2..W+1) {
        min_y = H + 1;
        while (!ss.empty && ss.front.x == x) {
            min_y = min(min_y, ss.front.y);
            segt.set(ss.front.y, 0);
            ss.popFront();
        }
        if (min_y == 1) xend = true;
        if (xend) {
            res += segt.allProd();
        } else if (min_y == H + 1) {
            res += H;
        } else {
            res += min_y - 1 + segt.prod(min_y + 1, H + 1);
        }
    }
    writeln(res);
}