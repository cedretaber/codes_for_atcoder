...
{
    int N; get(N);
    char[][] CS; get_lines(N, CS);

    auto G = MFGraph!int(N^^2 + 2);
    foreach (i; 0..N) foreach (j; 0..N) {
        int ii, jj;
        static foreach (d; [[0,1], [1,0], [0,-1], [-1,0]]) {
            ii = i + d[0];
            jj = j + d[1];
            if (0 <= ii && ii < N && 0 <= jj && jj < N) G.addEdge(i * N + j, ii * N + jj, 1);
        }
        if (CS[i][j] == C[(i + j) % 2]) {
            G.addEdge(N^^2, i * N + j, 2 * N^^2);
        } else if (CS[i][j] == C[(i + j + 1) % 2]) {
            G.addEdge(i * N + j, N^^2 + 1, 2 * N^^2);
        }
    }
    writeln(2 * N * (N - 1) - G.flow(N^^2, N^^2 + 1));
}
