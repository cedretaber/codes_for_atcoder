...
void main()
{
    int T; get(T);
    while (T--) {
        long X, Y, P, Q; get(X, Y, P, Q);
        auto res = long.max;
        foreach (t1; X..X+Y) foreach (t2; P..P+Q) {
            auto yz = crt([t1, t2], [2*X + 2*Y, P + Q]);
            if (yz[0] != 0 && yz[1] != 0) res = min(res, yz[0]);
        }
        if (res == long.max) {
            writeln("infinity");
        } else {
...
void main()
{
    ...
    while (T--) {
        ...
        foreach (t1; X..X+Y) foreach (t2; P..P+Q) {
            ...
            auto e = ext_gcd(x, y);
            if (e[2] < 0) continue;
            auto f = abs(t1 - t2) / e[2];
            res = min(res, x * e[0] * f + t1);
        }
        if (res == long.max) {
            writeln("infinity");
        } else {
