...
void main()
{
    ...
    auto ns = new int[][10^^5 + 1];
    foreach (i, h; HS) ns[h] ~= i.to!int;
    HS = [int.max] ~ HS ~ int.max;

    auto uft = Dsu(N + 1);
    auto res = new int[](N);
    foreach (nn; ns) if (!nn.empty) {
        foreach (i; nn) {
            if (HS[i] <= HS[i + 1]) uft.merge(i, i + 1);
            if (HS[i + 1] >= HS[i + 2]) uft.merge(i + 1, i + 2);
        }
        foreach (i; nn) res[i] = uft.size(i + 1) - 1;
    }
    writefln!"%(%d\n%)"(res);
}
