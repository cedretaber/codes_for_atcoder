...
void main()
{
    ...
    auto G = MCFGraph!(int, long)(N*2+2);
    auto s = 2*N, t = 2*N+1;

    G.addEdge(s, t, N*K, 10L^^9);
    foreach (i; 0..N) {
        G.addEdge(s, i, K, 0);
        G.addEdge(N+i, t, K, 0);
    }

    foreach (i; 0..N) foreach (j, a; readln.split.to!(long[])) {
        G.addEdge(i, (N+j).to!int, 1, 10L^^9 - a);
    }

    writeln(10L^^9 * (N*K).to!long - G.flow(s, t, N*K)[1]);
